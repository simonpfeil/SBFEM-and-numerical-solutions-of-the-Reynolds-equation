...
results1D_g = textread('results1D_g.txt');
results1D_theta = textread('results1D_theta.txt');
results2D_Pi = textread('results2D_Pi.txt');
results2D_p = textread('results2D_p.txt');

g_vec = results1D_g(1:n_x,1);
theta_vec = results1D_theta(1:n_x,1);
...
figure
hold on
plot(X_vec,g_vec(:,1),"Color","red")                                       % red: switch function
plot(X_vec,theta_vec(:,1),"Color","blue")                                  % blue: film fraction
xlim([0,360])
xticks([0,90,180,270,360])
xlabel('\itX\rm (deg)')
ylabel('\itg\rm (),  \it\vartheta\rm ()')
