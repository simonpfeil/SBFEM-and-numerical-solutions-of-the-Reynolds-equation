...
! **********************************************************************************************************
  ...
  DO iter = 1, iter_max																						! fixed-point iteration for linearization of the nonlinear boundary value problem (BVP); iter counts the iterations
    ...
    DO i = 1, n_pz																							! loop through pressure zones
      ...
      IF ( grooves .GE. 1 ) THEN																			! "IF oil supply grooves are present, ..." (otherwise, the steps below aren't necessary, since only Guembel is used for this case and we need a different formulation anyway, due to the singular mode resulting from the absence of oil supply BCs)
    ...
    DO i = 1, n_pz																							! loop through pressure zones
      ...
      IF ( grooves .EQ. 0 ) THEN																			! if no grooves exist, a zero eigenvalue is present, requiring a slightly different formulation (at the same time, we will exploit the fact that the solution without grooves is always conducted under Guembel conditions)
        ...
        C_vec(2:n_mod) = C_vec(2:n_mod)/(lambda_vec(2:n_mod)**2)                                            ! C = Lambda^-2 * Phi^T * R, using the intermediate result from the line above; note that we are excluding the singular mode
		C_vec(1) = 0.0d0                                                                                    ! no contribution of singular mode, assuming homogeneous BCs at the bearing boundary
		...
		Vec1(1) = 0.0d0																						! no contribution of singular mode
